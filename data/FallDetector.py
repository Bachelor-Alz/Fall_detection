...
class FallDetector:
    ...
    def fix_multiple_periods(self, value: str) -> str:
    ...
    def pre_process(self, df: pd.DataFrame):
        ...
        df_combined.loc[:, columns_to_scale] = scaler_combined.fit_transform(df_combined[columns_to_scale])
        save_location = os.path.join(os.pardir, 'combined_scaler.joblib')
        joblib.dump(scaler_combined, save_location)

        # Resample data to 50Hz (20ms interval) while handling duplicates and non-numeric columns
    ...
    def _resample_data(self, group: pd.DataFrame):
        ...
        datetime_data = numeric_group[datetime_cols]  
        numeric_data = numeric_group.drop(columns=datetime_cols)  
